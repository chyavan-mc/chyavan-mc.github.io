...
class Dot {
    ...
    draw() {
        if (this.visible) {
            ctx.fillStyle = calculateOpacity(this.xCoord, this.yCoord, 0.02, 0.15);
            ctx.beginPath();
            ...
            for (let i = 0; i < this.neighbors.length; i++) {
                ...
                if (this.neighbors[i].visible) {
                    ctx.strokeStyle = calculateOpacity((this.xCoord + this.neighbors[i].xCoord) / 2, (this.yCoord + this.neighbors[i].yCoord) / 2, 0.02, 0.15);
...
function drawMorse() {
    for (let i = 0; i < code.length; i++) {
        if (code[i] === "0") {
            ...
            ctx.fillStyle = calculateOpacity(xCoord + size, yCoord, 0.007, 0.08);
...
setInterval(createDotArray, 10000);