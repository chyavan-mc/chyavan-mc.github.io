...
const chooseBG = Math.random() > 0.01;
const distx = 80;
const disty = 100;
...
class Dot {
    ...
    draw() {
        if (this.visible) {
            ctx.fillStyle = calculateOpacity(this.xCoord, this.yCoord, 0.06, 0.15);
            ctx.beginPath();
...
function drawMorse() {
    for (let i = 0; i < code.length; i++) {
        if (code[i] === "0") {
            ...
            ctx.fillStyle = calculateOpacity(xCoord + size, yCoord, 0.03, 0.1);
